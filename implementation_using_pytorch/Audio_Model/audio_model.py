...
class MyDataset(Dataset):
    ...
    def __getitem__(self, audio_ind):
        ...
        if (sig_len > max_len):
            ...
            final_sig = np.float32(np.concatenate(
                (pad_begin, resig, pad_end), 0))
...
def train():
    ...
    # Random split of 80:20 between training and validation set
    num_items = len(myds)
    ...
    for e in range(100):
        ...
        for i, data in enumerate(train_dl):
            ...
            # Get the input features and target labesl, and put them on the GPU
            inputs, labels = data[0].to(device), data[1].to(device)
            inputs = inputs.unsqueeze(1)
            leaf_out = a(inputs.float())
            final_out = conv_model(leaf_out)
